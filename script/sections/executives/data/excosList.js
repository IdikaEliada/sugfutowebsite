import setEvent from './events.js';

export const excosList = [{
  name: "Aluta Solution",
  id: 27,
  about: "The 27th Elected SUG Executives of FUTO",
  logo: "https://i.ibb.co/WWmX9221/20251012-173114.png",
  color: 'blue',
  members: [{
    name: "Christopher Bukas",
    position: "President",
    about: "Leading with vision, excellence, and innovation — ",
    role: "Dedicated to progress, unity, and effective leadership, representing the voice and interest of all students as the President of the Aluta Solution Executives.",
    image: "https://i.ibb.co/LzWF0nvL/IMG-20251023-WA0085.jpg",
    titles: "HE. Bar. Hon. High Comr",
    id: {
      group: "SUGCORE",
      number: 1
    }
  }, {
    name: "Echeonwu Chidera",
    position: "Vice President",
    about: "A passionate advocate for students welfare, bringing vision, energy, and a collaborative spirit to the executive council. ",
    role: "Her leadership is focused on inclusion, unity, and growth.",
    image: "https://i.ibb.co/bjLrVLRD/IMG-20251023-WA0093.jpg",
    titles: "HE. High Comr.",
    id: {
      group: "SUGCORE",
      number: 2
    }
  }, {
    name: "Mark Christopher Ikechukwu",
    position: "Secretary General",
    about: "Diligent, organized, and committed to transparency, ensuring smooth communication and documentation within the association.",
    role: "He is hardworking and thorough, consistently putting in the effort to complete tasks accurately and on time.",
    image: "https://i.ibb.co/Z6T9FdbP/IMG-20251023-WA0089.jpg",
    titles: "Hon. Barr. High Comr.",
    id: {
      group: "SUGCORE",
      number: 3
    }
  }, {
    name: "Nduka Victory",
    position: "Director of Information",
    about: "Passionate about driving innovation and research, ensuring the community stays ahead on current happenings and relevant information",
    role: "",
    image: "https://i.ibb.co/zHn8rKG6/IMG-20251023-WA0095.jpg",
    titles: "Hon. Journ. High Comr.",
    id: {
      group: "SUGCORE",
      number: 4
    }
  }, {
    name: "Isaac Johnbosco",
    position: "Director of Welfare",
    about: "",
    role: "Dedicated to ensuring the well-being of the community, making sure every student feels supported and valued.",
    image: "https://i.ibb.co/1Gn0D09C/IMG-20251023-WA0091.jpg",
    titles: "High Comr.",
    id: {
      group: "SUGCORE",
      number: 5
    }
  }, {
    name: "Iheme Daniel",
    position: "Director of Sports",
    about: "",
    role: "Passionate about bringing energy and teamwork to the community through exciting sports initiatives.",
    image: "https://i.ibb.co/HTHQj845/IMG-20251023-WA0094.jpg",
    titles: "High Comr.",
    id: {
      group: "SUGCORE",
      number: 6
    }
  }, {
    name: "Ibim Great Graham",
    position: "Director of Socials",
    about: "",
    role: "The creative mind behind social events and student engagement, ensuring fun meets purpose in every gathering.",
    image: "https://i.ibb.co/MxW5R6bL/IMG-20251023-WA0090.jpg",
    titles: "High Comr.",
    id: {
      group: "SUGCORE",
      number: 7
    }
  }, {
    name: "Ugochukwu Chimakpa",
    position: "Director of Transport",
    about: "Staying in the loop and keeping everyone connected!",
    role: "Dedicated to keeping the community informed and connected with the latest updates and opportunities.",
    image: "https://i.ibb.co/wZKfZcbN/IMG-20251023-WA0086.jpg",
    titles: "Hon. High Comr.",
    id: {
      group: "SUGCORE",
      number: 8
    }
  }, {
    name: "Onubogu Chidera E.",
    position: "Financial Secretary",
    about: "",
    role: "Committed to managing funds with precision and supporting the community's growth.",
    image: "https://i.ibb.co/dwkGqK44/IMG-20251023-WA0088.jpg",
    titles: "High Comr.",
    id: {
      group: "SUGCORE",
      number: 9
    }
  }, {
    name: "Obi Jennifer Chisom",
    position: "Treasurer",
    about: "",
    role: "Committed to managing finances with care and transparency, ensuring the community thrives financially.",
    image: "https://i.ibb.co/B5SYkhfb/IMG-20251023-WA0078.jpg",
    titles: "High Comr.",
    id: {
      group: "SUGCORE",
      number: 10
    }
  }, {
    
    name: "Okafor Chioma Faithful",
    position: "Assistant Secretary General",
    about: "",
    role: " ",
    image: "https://i.ibb.co/M5g1mc4p/IMG-20251025-WA0074-1.jpg",
    titles: "High Comr.",
    id: {
      group: "SUGCORE",
      number: 11
    }
  }, {
    name: "Ezichi Franklyn Osugiri-Iro",
    position: "Director of Health ",
    about: "",
    role: "",
    image: "https://i.ibb.co/YFSyRd4c/IMG-20251023-WA0092.jpg",
    titles: "High Comr.",
    id: {
      group: "SUGCORE",
      number: 12
    }
  }
  ],
  events: setEvent(27)
 },{
  name: "Aluta Transformation",
  id: 26,
  about: "The 26th Elected SUG Executives of FUTO",
  logo: "https://i.ibb.co/xKKvmFxb/20251011-083515.png",
  color: 'green',
  members: [{
    name: "Ogbonna Blessed",
    position: "President",
    about: "Leading with vision, excellence, and innovation — ",
    role: "Dedicated to progress, unity, and effective leadership, representing the voice and interest of all students as the President of the 26th SUG Executives.",
    image: "https://i.ibb.co/zV8vZdSQ/20251009-171244.jpg",
    titles: "HE. Rt. Hon. High Comr",
    id: {
      group: "SUGCORE",
      number: 1
    }
  }, {
    name: "Isiguzo Priscilla",
    position: "Vice President",
    about: "A passionate advocate for students welfare, bringing vision, energy, and a collaborative spirit to the executive council. ",
    role: "Her leadership is focused on inclusion, unity, and growth.",
    image: "https://i.ibb.co/fYDXnDKn/20251009-175603.jpg",
    titles: "HE. High Comr.",
    id: {
      group: "SUGCORE",
      number: 2
    }
  }, {
    name: "Opara Prince Chukwuemeka",
    position: "Secretary General",
    about: "Diligent, organized, and committed to transparency, ensuring smooth communication and documentation within the association.",
    role: "He is hardworking and thorough, consistently putting in the effort to complete tasks accurately and on time.",
    image: "https://i.ibb.co/kVpKb270/20251009-213101.jpg",
    titles: "Hon. Rtr. Amb. High Comr.",
    id: {
      group: "SUGCORE",
      number: 3
    }
  }, {
    name: "Alajemba Paul Uzochukwu",
    position: "Director of Information",
    about: "Passionate about driving innovation and research, ensuring the community stays ahead on current happenings and relevant information",
    role: "",
    image: "https://i.ibb.co/cSPPH1K6/20251009-220855.jpg",
    titles: "Hon. Journ. High Comr.",
    id: {
      group: "SUGCORE",
      number: 4
    }
  }, {
    name: "Thunder Holy",
    position: "Director of Welfare",
    about: "",
    role: "Dedicated to ensuring the well-being of the community, making sure every student feels supported and valued.",
    image: "https://i.ibb.co/6JGVbP9Y/20251010-001837.jpg",
    titles: "Hon. High Comr.",
    id: {
      group: "SUGCORE",
      number: 5
    }
  }, {
    name: "Ndukwu Paul",
    position: "Director of Sports",
    about: "",
    role: "Passionate about bringing energy and teamwork to the community through exciting sports initiatives.",
    image: "https://i.ibb.co/nGwcSPV/20251010-002235.jpg",
    titles: "High Comr.",
    id: {
      group: "SUGCORE",
      number: 6
    }
  }, {
    name: "Nduka Anita",
    position: "Director of Socials",
    about: "",
    role: "The creative mind behind social events and student engagement, ensuring fun meets purpose in every gathering.",
    image: "https://i.ibb.co/xtzRqXhr/20251010-002359.jpg",
    titles:"High Comr.",
    id: {
      group: "SUGCORE",
      number: 3
    }
  }, {
    name: "Kanu Ifeanyi",
    position: "Director of Transport",
    about: "Staying in the loop and keeping everyone connected!",
    role: "Dedicated to keeping the community informed and connected with the latest updates and opportunities.",
    image: "https://i.ibb.co/Kp5XGHyd/20251010-003001.jpg",
    titles: "Hon. High Comr.",
    id: {
      group: "SUGCORE",
      number: 8
    }
  }, {
    name: "Aguocha Samuel Aguwhite",
    position: "Financial Secretary",
    about: "",
    role: "Committed to managing funds with precision and supporting the community's growth.",
    image: "https://i.ibb.co/RGj4nnhr/20251010-003801.jpg",
    titles: "High Comr.",
    id: {
      group: "SUGCORE",
      number: 9
    }
  }, {
    name: "Charles Princess",
    position: "Treasurer",
    about: "",
    role: "Committed to managing finances with care and transparency, ensuring the community thrives financially.",
    image: "https://i.ibb.co/vvjQtHLR/20251010-004013.jpg",
    titles: "High Comr.",
    id: {
      group: "SUGCORE",
      number: 10
    }
  }, {
      
    name: "Maduka Tracy",
    position: "Assistant Secretary General",
    about: "",
    role: " ",
    image: null,
    level: 200,
    titles: "High Comr.",
    id: {
      group: "SUGCORE",
      number: 11
    }

  }],
  events: setEvent(26)
},{
  name: "Aluta Proclaim",
  id: 25,
  about: "The 25th Elected SUG Executives of FUTO",
  logo: "https://i.ibb.co/fGTZ2V72/20251024-150525.png",
  color: 'orange',
  members: null,
  events: null,
  group: {
    image: "https://i.ibb.co/5gBMVB3D/IMG-20251024-WA0060.jpg",
    president: "H.E High Comr. Nwosu Sema Stanley"
  }
},{
  name: 'Aluta Legacy',
  id: 24,
  about: "The 24th Elected SUG Executives of FUTO",
  logo: "https://i.ibb.co/fzRxnYGX/20251024-151135.png",
  color: "blue",
  members: null,
  events: null,
  group: {
    image: "https://i.ibb.co/TDcFR43Z/IMG-20251024-WA0064.jpg",
    president: "H.E High Comr Ifeanyi Chinedu Christian"
  }
}
]